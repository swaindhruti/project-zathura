...
export const getUser = async (
    ...
    try {
        ...
        if (req.user?.role !== "ADMIN" && req.user?.id !== id) {
            return next(
                console.log("Not authorized to access this resource", 403)
            );
        }
...
export const updateUser = async (
    ...
    try {
        ...
        if (req.user?.id !== id) {
            return next(
                console.log("Not authorized to update this profile", 403)
            );
        }
...
export const deleteUser = async (
    ...
    try {
        ...
        if (req.user?.role !== "ADMIN" && req.user?.id !== id) {
            return next(
                console.log("Not authorized to delete this account", 403)
            );
        }
